...
public class MarketAggregator {
    ...
    private void execute() {
        ...
        LocalDate earliest = LocalDate.parse(candles.find().sort(SortingFactory.EARLIEST).limit(1).cursor().next().getString("date"));
        LocalDate latest = LocalDate.parse(candles.find().sort(SortingFactory.LATEST).limit(1).cursor().next().getString("date")).plusDays(1);
